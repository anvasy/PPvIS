...
public class SearchDayAndMonthStrategy implements SearchStrategy {
	...
	public ArrayList<Student> getArray(ComparableObject obj, ArrayList<Student> stud) {
		...
		for (int el = 0; el < stud.size(); el++) {
			if(obj.getMonthBirth() - 1 == stud.get(el).getBirthMonth() && obj.getDayBirth() == stud.get(el).getBirthDay()) {
